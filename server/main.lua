...
local function isNotBlacklisted(job)
    ...
    for _, word in ipairs(forbiddenWords) do
        if string.find(job, word) then
            DropPlayer(source, '[dqnut-jobcenter] blacklisted job')
            return false
        end
    end
    return true
...
local function isInRange(location)
    ...
    for _,loc in pairs(Config.Locations[location]) do
        ...
        if distance <= 10 then
            ...
            Wait(5000)
            DropPlayer(source, '[dqnut-jobcenter] trigger beveiliging')
        end
        return false
    end
end
...
local function addFlag(value, reason)
    ...
    if flags[source] then
        if flags[source] >= 3 then
            DropPlayer(source, '[dqnut-jobcenter] flagsystem')
        end
    end
    print('[INFO] Player: '..GetPlayerName(source)..' ('..source..') has been flagged. Total amount of flags: '..flags[source]..'')
end

RegisterNetEvent('dqnut-jobcenter:menu', function(location)
    ...
    for k, v in pairs(Config.Jobs[location]) do
        local element = {
            ...
            serverEvent = 'dqnut-jobcenter:server:hirePlayer',
    ...
    if isInRange(location) then
        TriggerClientEvent('dqnut-jobcenter:showSelection', source, {
            elements = elements,
            location = location
        })
    end
...
RegisterNetEvent('dqnut-jobcenter:server:hirePlayer', function(data)
    local xPlayer = ESX.GetPlayerFromId(source)
    local ped = GetPlayerPed(source)
    local coords = GetEntityCoords(ped)
